...
다음 예는 리스트와 딕셔너리가 지닌 함수(메서드)를 보여 주는 예
...
filter 함수는 첫 번째 인수로 함수를, 두 번째 인수로 그 함수에 차례로 들어갈 반복 가능한 데이터를 받는다.
...
`filter(positive, [1, -3, 2, 0, -5, 6])`는 `[1, -3, 2, 0, -5, 6]`의 각 요소값을 순서대로 positive 함수에 적용하여 반환 값이 참인 것만 묶어서 리턴한다.

즉 1, 2, 6 요소만 x > 0 문장이 참이 되므로 `[1, 2, 6]` 이라는 결괏값이 출력된다.
...
예를 들어 2진수로 표현된 "11"의 10진수 값은 다음과 같이 구할수 있다.
...
먼저 리스트의 첫 번째 요소인 1이 two_times 함수의 입력값으로 들어가고 `1 * 2`의 과정을 거쳐서 2가 된다. 다음으로 리스트의 두 번째 요소인 2가 `2 * 2` 의 과정을 거쳐 4가 된다.
